...
def make_avatar_center_adjust(x, y, z):
    ...
    anim.ensure_joint('Avatar Center', locKeysF=[[0.0, x*2.5, y*2.5, z*2.5], [1.0, 0.0, 0.0, 0.0]])
...
def make_avatar_center_bg():
    ...
    anim.ensure_joint('Avatar Center', locKeysF=[[0.0, 0.0, 0.0, 0.0]], rotKeysF=[[0.0, 0.0, 0.0, 0.0]])
...
def make_tail_adjust(z, y):
    ...
    anim.ensure_joint('mTail1', locKeysF=[[0.0, -y-0.116, 0.0, z+0.047]])
    anim.serialize_filename('tail_adjust_z%+.2f_y%+.2f.anim' % (z, y))


def make_tail_length(scale):
    anim = animDump.KeyframeMotion(priority=6, easeIn=0.0, easeOut=0.0)
    anim.ensure_joint('mTail2', locKeysF=[[0.0, -0.197*scale, 0.0, 0.0]])
    anim.ensure_joint('mTail3', locKeysF=[[0.0, -0.168*scale, 0.0, 0.0]])
    anim.ensure_joint('mTail4', locKeysF=[[0.0, -0.142*scale, 0.0, 0.0]])
    anim.ensure_joint('mTail5', locKeysF=[[0.0, -0.112*scale, 0.0, 0.0]])
    anim.ensure_joint('mTail6', locKeysF=[[0.0, -0.094*scale, 0.0, 0.0]])
